...
func Tasks() []Task {
	return []Task{
		{
			ID: "sql-intern-wallets", Title: "Кто на витрине", Grade: "intern", InternSafe: true,
			Story: "Старший на линии: «покажи кошельки, которые клиент видит в приложении — без холдов, просто available».",
			Question: "Выберите id, имя, валюту и available из intern_wallets.",
			Hint: "SELECT id, name, currency, available FROM intern_wallets;",
			Why: "На Intern аналитик сначала учится читать витрину, не трогая книгу.",
			Gold: "SELECT id, name, currency, available FROM intern_wallets",
		},
		{
			ID: "sql-intern-holds", Title: "Что отложено", Grade: "intern", InternSafe: true,
			Story: "Касса орёт SUCCESS, клиент орёт «деньги зависли». Старший: «посмотри холды».",
			Question: "Найдите холды со статусом OPEN в intern_holds.",
			Hint: "WHERE status = 'OPEN'",
			Why: "Холд — не проводка. Intern должен уметь отличить резерв от журнала.",
		...
		},
		{
			...
			Story: "Тикет tkt_hold: Дина не может заплатить в аптеке, в приложении минус. Операции просят сверку холда с книгой.",
			...
			Why: "На столе BA это классика: auth больше остатка из-за гонки или «забытого» холда. Клиенту показывают available, не ledger.",
			Gold: `SELECT id, customer_id, ledger, hold, (ledger - hold) AS available
FROM wallets
WHERE hold > 0 AND (ledger - hold) < 0`,
		},
		{
			ID: "sql-ui-mismatch", Title: "Экран врёт относительно книги", Grade: "junior",
			Story: "Елена пишет: «в приложении ноль, в отделении говорят, что деньги есть». Рядом таблица wallet_ui — то, что рисует фронт.",
			Question: "Найдите кошельки, где shown_available не равен ledger − hold (и shown_available задан).",
			Hint: "JOIN wallets и wallet_ui. NULL в витрине — «считай сам», это не ошибка.",
			Why: "Частый баг витрины: кэш available. Аналитик сверяет экран с книгой, не спорит с дизайнером.",
...
WHERE u.shown_available IS NOT NULL
  AND u.shown_available != (w.ledger - w.hold)`,
		...
		{
			ID: "sql-p2p-dup", Title: "P2P без ключа — два DR", Grade: "junior",
			Story: "Тикет tkt_p2p: Анна нажала перевод дважды. Idempotency-Key пустой. Учебный баг ядра: оба прошли.",
			...
			Why: "На работе это инцидент «дважды списали». Аналитик доказывает дубль запросом, не скрином чата.",
...
HAVING COUNT(*) > 1`,
		},
		{
			...
			Story: "Ночь сдала день: зарплатный хвост с чужим IBAN сидит в suspense. HR звонит «где деньги сотрудников».",
			Question: "Покажите suspense со статусом SUSPENSE, открытые больше 12 часов.",
			Hint: "created_at <= datetime('now', '-12 hours')",
			Why: "Невыясненные — чужие деньги. SLA старения — операционное требование, не «бухгалтер потом разберёт».",
...
WHERE status = 'SUSPENSE'
  AND created_at <= datetime('now', '-12 hours')`,
		...
		{
			ID: "sql-trial", Title: "Журнал не бьётся с пробным балансом", Grade: "middle",
			Story: "Контроль 1С: «по UZS дебет не равен кредиту». Кто-то провёл возврат одной ногой.",
			Question: "Посчитайте сумму DR, CR и diff по живым ногам ledger_legs в UZS.",
			Hint: "status != 'REVERSED'. diff = debit − credit. Ищите kind REFUND_HOLE.",
			Why: "Пробный баланс — первый контроль книги. Дыра на одной ноге — не «потом поправим в Excel».",
			Gold: `SELECT
...
WHERE currency = 'UZS' AND status != 'REVERSED'`,
		},
		{
			...
			Story: "Эквайринг: Cafe Orient орёт, что «касса зелёная, а выплат нет». Нужно сравнение auth и capture по мерчантам.",
...
ORDER BY merchant_account_id`,
		},
		{
			...
			Story: "Тикет tkt_fx: казначейство видит сделку 50 USD, в журнале ног нет. Позиция «голая».",
