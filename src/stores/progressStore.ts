    ...
    (set, get) => ({
      ...
      evaluateBadges: () => {
        ...
        for (const a of ACHIEVEMENTS) {
          ...
          if (a.id === "pet-bank" && s.modules["pet-wallet"]?.drills?.capturedHold) toUnlock.push(a.id);
          if (
            a.id === "pet-ledger" &&
            (s.modules["pet-wallet"]?.drills?.fxPosted ||
              s.modules["pet-wallet"]?.drills?.salaryPosted ||
              s.modules["pet-wallet"]?.drills?.clearingSettled)
          )
            toUnlock.push(a.id);
